...
import { MigrationBaseType } from '../types/migration.type';
      ...
      if (!helper.tableExists(TABLE_NAME)) {
        ...
        if (!helper.columnExists(TABLE_NAME, 'name')) {
          ...
          if (helper.columnType(TABLE_NAME, 'name') === DataTypes.STRING) {
            if (helper.columnLength(TABLE_NAME, 'name') < 1024) {
              await queryInterface.sequelize.query(
                `ALTER TABLE IF EXISTS "${ TABLE_NAME }" ALTER COLUMN "name" SET DATA TYPE VARCHAR(512)`,
                { transaction },
              );
            }
          }
